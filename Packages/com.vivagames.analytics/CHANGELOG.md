...
- The migration moves the common parameters of the old `FirebaseAnalytics.cs` into the new `AnalyticsInit.cs`, keeps any other custom line of the old scripts as comments in `OnFirebaseReady()` and writes `Assets/VivaAnalytics/Legacy/MIGRATION_NOTES.txt`.
